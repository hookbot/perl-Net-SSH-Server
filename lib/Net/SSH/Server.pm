...
sub session_file {
    ...
    return $ENV{SESSION_FILE} ||= $self->base."/session-$service-$id.env";
}
...
sub init_commandline_args {
    ...
    if (!$ENV{NET_SSH_OVERRIDE}) {
        ...
        if (@$dir or @$file > 1) {
            ...
            $ENV{NET_SSH_OVERRIDE} = $self->base."/".$self->pam_service().".conf";
...
sub banner_file {
    ...
    my $id = $self->{pam_id} ||= $ENV{PAM_ID};
    my $service = $self->pam_service;
    return $self->base."/banner-$service-$id.txt";
}
